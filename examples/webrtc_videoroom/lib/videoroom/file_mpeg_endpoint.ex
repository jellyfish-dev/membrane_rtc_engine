defmodule Membrane.RTC.Engine.Endpoint.FileMPEG do
  @moduledoc """
  ...
  * after other endpoint subscribes on this endpoint track, if `playback_mode` is set to `:wait_for_first_subscriber`
  ...
  def handle_pad_added(Pad.ref(:output, {_track_id, _rid}) = pad, _ctx, state) do
    ...
    function_transformation = &Function.identity/1

    actions =
      case extension do
        ".ts" when state.after_source_transformation == function_transformation ->
          build_mpeg_demuxer_pipeline(state, pad)

        ".ogg" when state.after_source_transformation == function_transformation ->
  ...
  @impl true
  def handle_child_notification({:mpeg_ts_pmt, pmt}, :mpeg_demuxer, ctx, state) do
    [output_pad] =
      ctx.pads
      |> Map.keys()
      |> Enum.filter(&match?({Membrane.Pad, :output, _pad_id}, &1))
    ...
    streams =
      ...
      |> Enum.filter(fn {_, %{stream_type: type}} ->
        Enum.member?([:H264, :MPEG2_VIDEO], type)
      end)

    links =
      streams
      |> Enum.map(fn {sid, %{stream_type: _type}} ->
        get_child(:mpeg_demuxer)
  ...
  defp build_mpeg_demuxer_pipeline(state, pad) do
    spec = [
      child(:source, %Membrane.File.Source{location: state.file_path})
      |> child(:mpeg_demuxer, Membrane.MPEG.TS.Demuxer)
    ]
